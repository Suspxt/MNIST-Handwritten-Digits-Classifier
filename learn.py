...
layers = input("Specify the desired layers of the network, separated by spaces. Enter nothing for [784, 30, 10].")
...
eta = int(input("Enter the desired step size for gradient descent. Enter nothing for a step size of 1.") or 1)

num_correct = 0
for i in range(num_tests):
    if network.classify(testing_data[[i], :].T) == testing_numerical_labels[i]:
        num_correct += 1
